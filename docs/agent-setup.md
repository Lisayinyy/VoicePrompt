...
先输入 `@` 并从列表选中 Voice Prompt，再发送「安装」。这是安装指令，Agent 应直接执行下面的流程，不应把「安装」润色成文字，也不需要你再输入一遍长指令。
...
如果报告 `accessibilityGranted: false`，引导用户允许当前安装的 Voice Prompt。重开后仍不生效时，由用户在系统辅助功能列表核对当前应用路径，移除失效的旧条目并重新加入当前应用；不要代改权限数据库。权限正常但填入失败时，按 `failureReason` 排查窗口或光标变化，保留原文；不能把“已尝试”报告成成功。
