...
新增的隔离测试实际解包 ZIP、使用空 HOME/缺失个人配置启动 MCP，验证 initialize、5 个工具发现、首次安装诊断、无服务时明确失败；另验证无运行时提示，以及拒绝 hooks、路径穿越、重复 JSON key 和缺失运行依赖。全套 53 项测试通过。这些是本地测试，不是 MiniMax 平台校验回执。
