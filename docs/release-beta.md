Voice Prompt 0.7.0 开发内测：新增 Qwen 本地识别，以及由 MiniMax Code Agent 引导的首次安装
...
Qwen 需要 Apple Silicon Mac / macOS 15+，模型与运行环境另需磁盘空间。本机 M2 Pro / 16GB 已做公开音频推理测试；38 项 Node 检查、Skill 验证和隔离目录的全新桌面安装检查通过。尚未验证另一台全新 Mac 的系统授权与真实输入链路，必须由测试者最终确认。
