...
subprocess.run(['swiftc', '-swift-version', '5', '-parse-as-library', '-target', 'arm64-apple-macosx13.0', '-O', str(root/'native/VoicePrompt.swift'), str(root/'native/VoicePromptUI.swift'), str(root/'native/VoiceHotkey.swift'), str(root/'native/VoiceDirectEdit.swift'), str(root/'native/VoicePasteboard.swift'), str(root/'native/VoiceInputGuard.swift'), str(root/'native/VoiceLivePreview.swift'), '-o', str(contents/'MacOS/VoicePrompt'), '-framework', 'AppKit', '-framework', 'AVFoundation', '-framework', 'Carbon'], check=True)
...
with (contents/'Info.plist').open('wb') as f:
 plistlib.dump({'CFBundleExecutable':'VoicePrompt','CFBundleIdentifier':'ai.voiceprompt.desktop','CFBundleName':'Voice Prompt','CFBundleDisplayName':'Voice Prompt','CFBundlePackageType':'APPL','CFBundleShortVersionString':'0.7.0','CFBundleVersion':'17','LSMinimumSystemVersion':'13.0','LSUIElement':True,'NSMicrophoneUsageDescription':'Voice Prompt records speech only when you start dictation, then transcribes it locally.','NSHighResolutionCapable':True}, f)
