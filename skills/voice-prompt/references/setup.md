...
Open the desktop AI 润色 page and wait for automatic free setup. New users do not need an invitation, API key or their own provider; the publisher's MiniMax key stays on the HTTPS backend and each Mac keeps only its device credential in Keychain. Preserve existing custom providers unless the user asks to switch. If automatic setup is unavailable, distinguish the service being unavailable from this Mac being offline or awaiting Keychain permission. Do not ask default free-service users to fill Provider settings, enter a personal API key, or reinstall to fix a server outage. Use 重试 after the actual issue is resolved; never hide the outage or claim AI is ready before a real model call succeeds. Do not copy the publisher's configuration. Automatic setup verifies a real model call before saving; confirm fallback=false for the user's test. Free service allows 30 calls per device per day and is subject to shared capacity. Do not promise unlimited usage or that API success proves input insertion.
...
- `unconfirmed` means the direct input was attempted but no readback confirmed it. Ask whether text actually appeared before retrying, to avoid duplicates.
